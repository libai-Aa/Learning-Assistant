 ...
 * @description PDF引用验证技能，验证PDF中的引用是否存在且准确
 * @module src/lib/aminer/pdf-citation-verifier
 */
...
/**
 * AMiner PDF Citation Verifier Skill
 * @description 提供PDF引用验证功能，验证PDF中的引用是否真实存在且准确
 */
export class PdfCitationVerifier extends AminerSkillBase {
  ...
  async verify(
    ...
    const requestParams: Record<string, unknown> = {
      ...
      verify_existence: params.verifyExistence !== false
    }
  ...
  async getNonExistentCitations(
    ...
    return response.verifications.filter(v => v.exists === false)
  }
  ...
  async getPdfMetadata(
    pdfPath: string,
    citations?: string[]
  ): Promise<PdfCitationVerificationResponse['pdfMetadata'] | null> {
    const response = await this.verify({
      pdfPath,
      citations: citations || ['__metadata_only__'], // 占位引用以通过参数校验
      extractText: false
    })

    return response.pdfMetadata || null
  }
}
