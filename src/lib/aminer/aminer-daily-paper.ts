 ...
 * @description 每日论文推荐技能，根据兴趣主题、学者、账号获取最新论文
 * @module src/lib/aminer/aminer-daily-paper
 */
...
/**
 * AMiner Daily Paper Skill
 * @description 提供每日论文推荐功能，支持基于主题、学者、账号的个性化推荐
 */
export class AminerDailyPaper extends AminerSkillBase {
  ...
  async getDailyPapers(
    ...
    if (params.accountIds && params.accountIds.length > 0) {
      requestParams.account_ids = params.accountIds
    }
  ...
  /**
   * 根据学者获取推荐论文
   * @param scholarIds 学者ID列表
   * @param limit 返回数量
   * @returns 论文列表
   */
  async getPapersByScholars(
    scholarIds: string[],
    limit: number = 10
  ): Promise<AcademicSearchResult[]> {
    const response = await this.getDailyPapers({ scholarIds, limit })
    return response.papers
  }
   ...
   * @returns 论文列表
   */
  async getPapersByAccounts(
    accountIds: string[],
    limit: number = 10
  ): Promise<AcademicSearchResult[]> {
    const response = await this.getDailyPapers({ accountIds, limit })
    return response.papers
  }
